	...
	private static Student processStudent(String line){
			...
			try{
				...
				if(itemCount < 5 || itemCount > 6){
					...
					student = new Student(lineScanner.next(), lineScanner.next(), lineScanner.next(), lineScanner.next(), lineScanner.next());
				}
				else{
					//item count is 6
					//if 5 strings and int then 6 construtor can be used
					student = new Student(lineScanner.next(), lineScanner.next(), lineScanner.next(), lineScanner.next(), lineScanner.next(), lineScanner.nextInt());
					}	
			} catch (Exception e){
				//skip
			}
